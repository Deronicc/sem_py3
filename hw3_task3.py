...
dictionary_ru = {1: ['А', 'В', 'Е', 'И', 'Н', 'О', 'Р', 'С', 'Т'], 2: ['Д', 'К', 'Л', 'М', 'П', 'У'], 3: ['Б', 'Г', 'Ё', 'Ь', 'Я'], 4: ['Й', 'Ы'], 5: ['Ж', 'З', 'Х', 'Ц', 'Ч'], 8: ['Ш', 'Э', 'Ю'], 10: ['Ф', 'Щ', 'Ъ']}
...
for i in word:
    if i in "АБВГДЕЁЖЗИЙКЛМНОПРСТУФХЦЧШЩЪЫЬЭЮЯ":
        for j in dictionary_ru:
            if i in dictionary_ru[j]:
                count = count + j
                print("Буква в слове: ", i , "стоимость: ", j)
                break
    else:
        for j in dictionary_eng:
            if i in dictionary_eng[j]:
                count = count + j
                print("Буква в слове: ", i , "стоимость: ", j)
                break
